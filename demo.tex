...
The way it works is by setting the document class to \texttt{article} for the HTML version, and redefining the \texttt{frame} environment so that each frame becomes a section. Together with some appropriate CSS tweaks, we end up with an HTML document with one page per frame, which behaves likes slides.
...
\begin{itemize}
    ...
    \item If you already have \verb|\section| commands in your slides, they will each produce their own separate slide.
